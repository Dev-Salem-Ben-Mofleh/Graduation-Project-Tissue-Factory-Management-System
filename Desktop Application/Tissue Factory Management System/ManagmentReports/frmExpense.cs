...
namespace WindowsFormsApp1.ManagmentReports
{
    public partial class frmExpense : Form
    {
        ...
        private static System.Data.DataTable _dtAllReports;

        private System.Data.DataTable _dtPeople;

        private void calculate(System.Data.DataTable dt)
        {
            decimal totalSum = 0;
        ...
        {
            _dtAllReports = clsExpense.GetReports(typeofSearch, date);
            ...
            _dtPeople = _dtAllReports.DefaultView.ToTable(false, "ExpenseDate", "Total", "CountBills",
            "Amount", "Name");
            ...
            lblRecordsCountStocks.Text = dgvSaleR.Rows.Count.ToString();
                ...
                dgvSaleR.Columns[1].HeaderText = "الأجمالي ";
                ...
                dgvSaleR.Columns[3].HeaderText = "المبلغ المدفوع";
                dgvSaleR.Columns[3].Width = 120;

                dgvSaleR.Columns[4].HeaderText = "نوعالمصروف";
        ...
        private void _GenerateClientReport(bool saveAsExcel)
        {
            ...
            string templatePath = @"F:\تقارير المصروفات.xlsx";

            string fileExtension = saveAsExcel ? ".xlsx" : ".pdf";
            string outputPath = $@"F:\مشروع التخرج\Expenses Report\تقارير المصروفات_ال{cbTypeMove.Text}_لتاريخ_{date}{fileExtension}";
            ...
            using (ExcelPackage package = new ExcelPackage(fileInfo))
            {
                ...
                foreach (DataGridViewRow Row in dgvSaleR.Rows)
                {
                    ...
                    worksheet.Cells[row, 4].Value = Row.Cells[4].Value.ToString();
                    worksheet.Cells[row, 5].Value = Row.Cells[3].Value.ToString();
        ...
        {
            _dtAllReports = clsExpense.GetReports("يومي", dtFrom.Value);
            if (_dtAllReports.DefaultView.Count == 0)
            {
                return;
            }
            ...
            foreach (DataRow row in _dtPeople.Rows)
            {
                ...
                try
                {
                    HttpResponseMessage response = await client.PostAsync("https://tissue.runasp.net/api/Tissue/AddExpenseReport", content);
        ...
        public async Task<bool> CheckIfSaleExistsAsync(DateTime date)
        {
            ...
            string url = $"https://tissue.runasp.net/api/Tissue/existsEx/{formattedDate}";
