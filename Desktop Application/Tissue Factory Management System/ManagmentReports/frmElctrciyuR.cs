...
{
    public partial class frmElctrciyuR : Form
    {
        public frmElctrciyuR()
        {
            InitializeComponent();
        }
        DateTime date;
        ...
        {
            _dtAllReports = clsElectricite.GetReports(typeofSearch, date);
            ...
            _dtPeople = _dtAllReports.DefaultView.ToTable(false, "date", "TypeOf", "Total",
            "Quantity");
            ...
            lblRecordsCountStocks.Text = dgvSaleR.Rows.Count.ToString();
                ...
                dgvSaleR.Columns[1].HeaderText = "نوع النفط";
                dgvSaleR.Columns[1].Width = 150;

                dgvSaleR.Columns[2].HeaderText = "أجمالي المبلغ";
                dgvSaleR.Columns[2].Width = 100;


                dgvSaleR.Columns[3].HeaderText = "عدد اللترات";
        ...
        private void frmElctrciyuR_Load(object sender, EventArgs e)
        {
        ...
        private void _GenerateClientReport(bool saveAsExcel)
        {
            ...
            string templatePath = @"F:\تقارير الكهرباء.xlsx";

            string fileExtension = saveAsExcel ? ".xlsx" : ".pdf";
            string outputPath = $@"F:\مشروع التخرج\Electricty Report\تقارير الكهرباء_ال{cbTypeMove.Text}_لتاريخ_{date}{fileExtension}";
        ...
        {
            _dtAllReports = clsElectricite.GetReports("يومي", dtFrom.Value);
            if (_dtAllReports.DefaultView.Count == 0)
            {
                dgvSaleR.Columns.Clear();
                lblnotfound.Visible = true;
            ...
            foreach (DataRow row in _dtPeople.Rows)
            {
                ...
                try
                {
                    HttpResponseMessage response = await client.PostAsync("https://tissue.runasp.net/api/Tissue/AddEleReport", content);
        ...
        public async Task<bool> CheckIfSaleExistsAsync(DateTime date)
        {
            ...
            string url = $"https://tissue.runasp.net/api/Tissue/existsE/{formattedDate}";
