...
{
    public partial class frmBurncheR : Form
    {
        public frmBurncheR()
        {
            InitializeComponent();
        ...
        {
            _dtAllReports = clsPurchase.GetAllPurchases(typeofSearch, date);
            ...
            "RawMatiral", "Name", "PaidBill", "NotPaidBill");
            ...
            lblRecordsCountStocks.Text = dgvSaleR.Rows.Count.ToString();
                ...
                dgvSaleR.Columns[1].HeaderText = "أجمالي المشتريات";
                ...
                dgvSaleR.Columns[3].HeaderText = "أسم المادة المستوردة";
                dgvSaleR.Columns[3].Width = 120;

                dgvSaleR.Columns[4].HeaderText = "أسم المورد";
        ...
        private void frmBurncheR_Load(object sender, EventArgs e)
        {
        ...
        private void _GenerateClientReport(bool saveAsExcel)
        {
            ...
            string templatePath = @"F:\تقارير المشتريات.xlsx";

            string fileExtension = saveAsExcel ? ".xlsx" : ".pdf";
            string outputPath = $@"F:\مشروع التخرج\Pruches Report\تقارير المشتريات_ال{cbTypeMove.Text}_لتاريخ_{date}{fileExtension}";
        ...
        {
            _dtAllReports = clsPurchase.GetAllPurchases("يومي", dtFrom.Value);
            if (_dtAllReports.DefaultView.Count == 0)
            {
                return;
            }
            ...
            foreach (DataRow row in _dtPeople.Rows)
            {
                ...
                { 
                    ...
                    rawMatiral = row["RawMatiral"].ToString(),
                ...
                try
                {
                    HttpResponseMessage response = await client.PostAsync("https://tissue.runasp.net/api/Tissue/AddPruchaseReport", content);
        ...
        public async Task<bool> CheckIfSaleExistsAsync(DateTime date)
        {
            ...
            string url = $"https://tissue.runasp.net/api/Tissue/existsP/{formattedDate}";
