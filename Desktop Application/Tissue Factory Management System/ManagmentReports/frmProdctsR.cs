...
{
    public partial class frmProdctsR : Form
    {
        public frmProdctsR()
        {
            InitializeComponent();
        ...
        {
            _dtAllReports = clsProduction.GetReports(typeofSearch, date);
            ...
            _dtPeople = _dtAllReports.DefaultView.ToTable(false, "ProductionDate", "ProductName", "Quantity",
            "DamagedQuantity", "Material_Name", "RawAmount");
            ...
            lblRecordsCountStocks.Text = dgvSaleR.Rows.Count.ToString();
                ...
                dgvSaleR.Columns[1].HeaderText = "أسم المنتج";
                dgvSaleR.Columns[1].Width = 150;

                dgvSaleR.Columns[2].HeaderText = "الكمية المنتجة";
                dgvSaleR.Columns[2].Width = 100;


                dgvSaleR.Columns[3].HeaderText = "الكمية التالفة";
                dgvSaleR.Columns[3].Width = 120;

                dgvSaleR.Columns[4].HeaderText = "أسم المادة المستهلكة";
                dgvSaleR.Columns[4].Width = 100;

                dgvSaleR.Columns[5].HeaderText = "الكمية المستهلكة";
        ...
        private void frmProdctsR_Load(object sender, EventArgs e)
        {
        ...
        private void _GenerateClientReport(bool saveAsExcel)
        {
            ...
            string templatePath = @"F:\تقارير المنتجات.xlsx";

            string fileExtension = saveAsExcel ? ".xlsx" : ".pdf";
            string outputPath = $@"F:\مشروع التخرج\Production Report\تقارير المنتجات_ال{cbTypeMove.Text}_لتاريخ_{date}{fileExtension}";
        ...
        {
            _dtAllReports = clsProduction.GetReports("يومي", dtFrom.Value);
            if (_dtAllReports.DefaultView.Count == 0)
            {
            ...
            foreach (DataRow row in _dtPeople.Rows)
            {
                var Report = new
                {
                    productionDate = Convert.ToDateTime(row["ProductionDate"]),
                    quantity = Convert.ToInt32(row["Quantity"]),
                    rawAmount = Convert.ToInt32(row["RawAmount"]),
                ...
                try
                {
                    HttpResponseMessage response = await client.PostAsync("https://tissue.runasp.net/api/Tissue/AddProudoctionReport", content);
        ...
        public async Task<bool> CheckIfSaleExistsAsync(DateTime date)
        {
            ...
            string url = $"https://tissue.runasp.net/api/Tissue/existsPr/{formattedDate}";
