...
{
    public partial class frmRawmaR : Form
    {
        public frmRawmaR()
        {
            InitializeComponent();
        ...
        {
            _dtAllReports = clsRawMaterial.GetReports(typeofSearch, date);
            ...
            _dtPeople = _dtAllReports.DefaultView.ToTable(false, "ProductionDate", "Material_Name", "RawAmount",
            "Quantity");
            ...
            lblRecordsCountStocks.Text = dgvSaleR.Rows.Count.ToString();
                ...
                dgvSaleR.Columns[1].HeaderText = "أسم المادة الخام";
                dgvSaleR.Columns[1].Width = 150;

                dgvSaleR.Columns[2].HeaderText = "الكمية المستهلكة";
                dgvSaleR.Columns[2].Width = 100;

                dgvSaleR.Columns[3].HeaderText = "الكمية المتبقية";
        ...
        private void frmRawmaR_Load(object sender, EventArgs e)
        {
        ...
        private void _GenerateClientReport(bool saveAsExcel)
        {
            ...
            string templatePath = @"F:\تقارير المواد الخام.xlsx";

            string fileExtension = saveAsExcel ? ".xlsx" : ".pdf";
            string outputPath = $@"F:\مشروع التخرج\RawMatirals\تقارير المواد_الخام_ال{cbTypeMove.Text}_لتاريخ_{date}{fileExtension}";
        ...
        {
            _dtAllReports = clsRawMaterial.GetReports("يومي", dtFrom.Value);
            if (_dtAllReports.DefaultView.Count == 0)
            {
                return;
            }
            ...
            foreach (DataRow row in _dtPeople.Rows)
            {
                ...
                try
                {
                    HttpResponseMessage response = await client.PostAsync("https://tissue.runasp.net/api/Tissue/AddRaawMatirailsReport", content);
        ...
        public async Task<bool> CheckIfSaleExistsAsync(DateTime date)
        {
            ...
            string url = $"https://tissue.runasp.net/api/Tissue/existsR/{formattedDate}";
